        ...
        addMouseListener(new MouseListener()
        {
            ...
            public void mousePressed(MouseEvent e)
            {
                if(m_arLogInfo != null)
                {
                    ...
                    if(nIndex < 0) {
                        System.out.printf("nIndex[%d] = size[%d]*rate[%f]\n", nIndex, m_arLogInfo.size(), fRate);
                        return;
                    }
        ...
        addMouseMotionListener(new MouseMotionListener()
        {
            ...
            public void mouseDragged(MouseEvent e)
            {
                if(m_arLogInfo != null)
                {
                    ...
                    System.out.printf("mouseDragged : m_arLogInfo null");
                }
            }
        });
...
//        }
    ...
    {
        if(m_arLogInfo == null) {
            System.out.printf("drawIndicator : m_arLogInfo null");
            return;
        }
        ...
        if(TOTAL_COUNT > 0)
        {
            ...
            for( Integer nIndex : m_hmBookmark.keySet())
            {
                if(m_LogFilterMain.m_nChangedFilter == LogFilterMain.STATUS_CHANGE || m_LogFilterMain.m_nChangedFilter == LogFilterMain.STATUS_PARSING)
                    break;
                if(m_hmBookmark.containsKey(nIndex))
                {
            ...
            for( Integer nIndex : m_hmError.keySet())
            {
                if(m_LogFilterMain.m_nChangedFilter == LogFilterMain.STATUS_CHANGE || m_LogFilterMain.m_nChangedFilter == LogFilterMain.STATUS_PARSING)
                    break;
                if(m_hmError.containsKey(nIndex))
                {
                    try {
    ...
    {
//        System.out.printf("drawError : %d %d %d %d\n", m_rcError.x, m_rcError.y, m_rcError.width, m_rcError.height);
        g.setColor(Color.RED);
    ...
    {
        if(m_arLogInfo == null) {
            System.out.printf("drawPageIndicator : m_arLogInfo null\n");
            return;
        }
