...
public abstract class RecentFileMenu extends JMenu{
	 ...
	 */
	public RecentFileMenu(String name, int count, JComboBox combobox, int type){
		super();
        String saveDir = "ini";
		this.setText("Recent");
		this.setMnemonic('R');
		...
		if((this.pathToSavedFile==null)||(this.pathToSavedFile.length()<=0)){
            ...
            if(this.pathToSavedFile.endsWith(File.separator)) {
    			this.pathToSavedFile=this.pathToSavedFile+saveDir;
    		} else{
    			this.pathToSavedFile=this.pathToSavedFile+File.separator+saveDir;
    		}
//            System.out.println("dirPath : " + this.pathToSavedFile);

            File theDir = new File(this.pathToSavedFile);
		...
		if(recentFile.exists()){
			try{
				...
				while(reader.ready()){
					this.addEntry(reader.readLine(),false, combobox);
				}
			} catch(Exception x){
				x.printStackTrace();
			}
	...
	private void addEntry(String filePath,boolean updateFile){
		this.addEntry(filePath, true, null);
    }
	/**
	 * Adds a new entry to the menu. Moves everything "down" one row.
	 * @param filePath The new path to add.
	 ...
	 */
	private void addEntry(String filePath,boolean updateFile, JComboBox combobox){
	...
	//		if(!this.recentEntries[index-1].equalsIgnoreCase(filePath))
			if(!this.recentEntries[index-1].equals(filePath))
      {
...
//		if(!this.recentEntries[0].equalsIgnoreCase(filePath))
		if(!this.recentEntries[0].equals(filePath))
    {
            ...
            if(combobox != null)
            {
                ...
                combobox.insertItemAt(filePath, 0);
            }
		}
		...
		for(int index=0;index<this.itemCount;index++)
        {
            ...
            if(this.recentEntries[index].equals(defaultText)){
                ...
                public void actionPerformed(ActionEvent actionEvent){
                    onSelectFile(type, actionEvent.getActionCommand());
                    }
                    });
            }
            this.add(menuItem);
	 ...
	 */
	public abstract void onSelectFile(int type, String filePath);
}
